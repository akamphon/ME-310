...
\begin{solution}
  ...
  \begin{figure}[H]
    ...
    \begin{tikzpicture}
      \begin{axis}[
        ...
        \addplot [domain=0.75:1, DarkGrey!20!Black] {250*(1-x)*cos(15)} node[midway, above right]{long};
      \end{axis}
    \end{tikzpicture}
  \end{figure}
  ...
  \begin{align*}
    S &= \frac{N M}{S_y} = \frac{\py{N}(\py{round(M,1)})}{\py{sigma_y}} \\
      &= \py{'{:.2E}'.format(S)} \text{ m}^3
  \end{align*}

  We can look up a catalog to determine a proper cross section for the beam.

  Now, for the column, we can determine the proper section from 
\end{solution}
