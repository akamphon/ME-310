...
\begin{solution}
  ...
  The bending moment caused by the midpoint load is $FL/8$ = \py{int(M)} N-m. Refering to the S-shaped steel, we can generate results using interaction formula \cref{eqn: interaction formula}.
...
The moment of inertia $I$ of a group of bolts about a centroid can be calculated by summing all the individual moments of inertia about that centroid $I_i$.

\begin{align}
  \label{eq: moment of inertia of bolts}
  I &= \sum I_i \\
  I_i &= I_{ci} + A_ir_i^2 = \frac{\pi}{64}r_i^4 + \frac{\pi}{4}d_i^2 r_i^2
\end{align}

where $A_i, d_i,$ and $r_i$ are the cross-sectional area, bolt diameter, and distance from the centroid of bolt $i$. In most bolt patterns, the centroidal moment of inertia $I_{ci}$ is much smaller than the $A_ir_i^2$, thus, the equation can be simplified to.

\begin{align}
  I_i = \frac{\pi}{4}d_i^2 r_i^2
\end{align}

If the bolts are identical, the equation for moments of inertia and resultant forces reduce to
...
\subsection{Bolted Joints under Torsion}
