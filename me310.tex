...
\begin{example} Design of an axially loaded beam with bending
  ...
  \begin{figure}[H]
    \centering
    \begin{tikzpicture}[>=latex]
      \node [draw, top color=lightblue, bottom color=lightblue, middle color=lightblue!20, minimum height=6mm, minimum width=7cm](beam){};
      % supports
      \node at (beam.south east) [anchor=north, draw, fill=LightGrey, circle, minimum height=4mm, inner sep=0] (B){};
      \node at (beam.south west) [anchor=north, draw, fill=LightGrey, regular polygon, regular polygon sides=3, minimum height=5mm, inner sep=0] {};
      % dimensions
...
\begin{solution} State of stress of an axially loaded beam with bending
  ...
  $\sigma_{total} = 14.15 + 9.43$ = \py{14.15+9.43} MPa.
\end{solution}
...
\begin{example} Design of a luffing crane
  ...
  \begin{figure}[H]
    ...
    \caption{a typical luffing crane \cite{monokloncrane}}
    \label{tower crane}
  \end{figure}
...
\item We need to determine the load carried by the front and rear wheels and their respective forks.
  ...
  \begin{pycode}
    ...
    print(r'\begin{align*}')
    print(r'A_{tf} = %.2f \\' %A_tf)
    print(r'A_{tr} = %.2f' %A_tr)
    print(r'\end{align*}')
  \end{pycode}
...
\item Since there are two identical springs connected in parallel, the loads are shared evenly. The maximum force on each spring then would be 1000/2 = 500 N. And for every 100/2 = 50 N, the indicator should be 2 cm apart, telling us that the spring constant $k$ should be 50/0.02 = 2500 N/m.
  ...
  \begin{pycode}
    ...
    print(r'\begin{align*}')
    print(r'd = %d \text{ mm}' %d)
    print(r'\end{align*}')
  \end{pycode}

  So the diameter of the wire is 14.8 cm. Finally, to determine the required number of active coils, we use
